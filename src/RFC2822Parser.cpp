...
void RFC2822Parser<R, W>::get_content(content_t *content)
{
    ...
    if (!this->start_with(content_type, M::CONTENT_TYPE)) {
        DUMP_INPUT_AND_THROW(MESSAGE_RFC2822_FORMAT_INVALID);
    }
    ...
    if (start == NULL) {
        ...
        start = this->get_param(start, content_type+strlen(content_type),
...
void RFC2822Parser<R, W>::skip_header()
{
    ...
    while ((M::fill() != 0) || (M::buffer_.get_size() != 0)) {
        ...
        M::buffer_.erase(this->skip_line(M::buffer_.get_data()) -
                         M::buffer_.get_data());
    }
...
void RFC2822Parser<R, W>::parse_header()
{
    ...
    while ((M::fill() != 0) || (M::buffer_.get_size() != 0)) {
        ...
        if (this->start_with(M::buffer_.get_data(), M::CONTENT_TYPE)) {
            if (content_type == NULL)  {
                content_type = M::buffer_.get_data();
                content_type_end = this->skip_line(M::buffer_.get_data()) -
                    strlen(M::CR_LF);
                break;
            }
        }

        start = this->skip_line(M::buffer_.get_data());
    ...
    if (this->get_param(start, content_type_end, M::BOUNDARY_PARAM,
                  &boundary) == NULL) {
